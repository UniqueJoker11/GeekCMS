...
/*选择判断框*/
zoom.closeChooseItemDialog=function(){
	var closeChooseItemDialog = document.getElementById("chooseItemDialog");
	if(closeChooseItemDialog != null) window.document.body.removeChild(closeChooseItemDialog);
	
}
zoom.showChooseItemDialog=function(content,title,confirmCon,cancelCon,confirmFun,cancelFun){
	if(title==null)title="消息选择提示";
	var dialog_div=document.createElement("div");
	$(dialog_div).attr("id","chooseItemDialog").addClass("dialog");
	var dialogContent="<div class=\"dialogFullScreen\"></div>"+
	"<div class=\"dialogContent dialogBackgroundColor\">"+
	"<div class=\"dialogPanel dialogFontColor\">"+
		"	<h1>"+title+"</h1>"+
      "  <div class=\"showInputDialogPanel\">"+
       ...
       " </div>"+
        "<div class=\"dialogButton\">"+
        	"<button id=\"dialogButtonConfirm\">"+confirmCon+"</button> &nbsp; &nbsp; "+
        	"<button id=\"dialogButtonCancel\">"+cancelCon+"</button>"+
       " </div>"+
  	"</div>"+
	"</div>";
	$(dialog_div).html(dialogContent);
	window.document.body.appendChild(dialog_div);
	$("#chooseItemDialog").focus();//设置焦点
	$("#chooseItemDialog").bind("keyup",function(event){
		if(event.keyCode == 13) fun();
	});
	$("#dialogButtonConfirm").bind("click",confirmFun);
	$("#dialogButtonCancel").bind("click",cancelFun);
	}
/* 输入框框 */
